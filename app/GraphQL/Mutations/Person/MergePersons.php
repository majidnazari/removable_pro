...
final class MergePersons
{
    ...
    private function mergeMarriages($primaryPersonId, $secondaryPersonId, $user_id)
    {
        ...
        })->each(function ($marriage) use ($primaryPersonId, $secondaryPersonId, $user_id) {
    ...
    private function mergeChildren($primaryPersonId, $secondaryPersonId,$user_id)
    {
        ...
        })->each(function ($child) use ($primaryPersonId, $secondaryPersonId,$user_id) {
    ...
    private function cleanupDuplicates($user_id)
    {
        // Clean up duplicate marriages across the entire PersonMarriage table
        PersonMarriage::all()->groupBy(function ($marriage) {
            return $marriage->man_id . '-' . $marriage->woman_id;
        })->each(function ($duplicates) use ($user_id) {
            $duplicates->shift(); // Keep the first record in each group
